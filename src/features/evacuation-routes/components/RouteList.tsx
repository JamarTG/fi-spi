...
const RouteList: React.FC<RouteListProps> = ({
  ...
  {filteredRoutes.map((route) => (
    <div
      ...
      className={`mb-2 p-2 rounded text-[11px] cursor-pointer ${
        selectedRoute === route.id ? "bg-blue-50" : "bg-gray-50"
      }`}
      style={{
        border: `2px solid ${getRouteColor(route.priority, route.status)}`,
      }}
    >
      ...
      <div className="flex justify-between mb-1">
        <span>
          Status:{" "}
          <span style={{ color: getRouteColor(route.priority, route.status) }}>
      ...
      <div className="flex justify-between mb-1">
        ...
        <span style={{ color: getHazardColor(route.hazardLevel) }}>
          {route.hazardLevel} risk
        </span>
      </div>
      {simulationMode && (
        <div className="mt-1">
          <div className="text-[10px] mb-0.5">
            Evacuated: {(evacuationProgress[route.id] || 0).toFixed(0)}%
          </div>
          <div className="w-full h-1 bg-gray-300 rounded-sm">
            <div
              ...
              style={{
                ...
                backgroundColor: getRouteColor(route.priority, route.status),
              }}
            ></div>
          </div>
        </div>
