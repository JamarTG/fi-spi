...
>
        ...
        <Popup>
          <div className="min-w-[200px]">
            ...
            <p>
              ...
              <span style={{ color: getHazardColor(route.hazardLevel) }}>{route.hazardLevel}</span>
            </p>
            {simulationMode && (
              <div className="mt-2 p-1 bg-gray-100 rounded-sm">
                <strong>Evacuation Progress:</strong>{" "}
                {(evacuationProgress[route.id] || 0).toFixed(1)}%
              </div>
            )}
          </div>
        </Popup>
  ...
  {showShelters &&
    emergencyShelters.map((shelter) => (
      <Marker key={`shelter-${shelter.id}`} position={[shelter.lat, shelter.lng]}>
        <Popup>
          <div className="min-w-[200px]">
            ...
            <p>
              <strong>Status:</strong>{" "}
              <span
                className={shelter.status === "operational" ? "text-green-600" : "text-orange-500"}
              >
                {shelter.status}
              </span>
            </p>
